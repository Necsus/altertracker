...
def getToken() -> str:
    ...
    if _token:
        return _token
    try:
        _token0 = CookieManager.query.filter_by(name="__Secure-next-auth.session-token.0").first()
        _token1 = CookieManager.query.filter_by(name="__Secure-next-auth.session-token.1").first()
        headers = {
            "Cookie": f"__Secure-next-auth.callback-url=https%3A%2F%2Fwww.altered.gg;__Secure-next-auth.session-token.0={_token0.value};__Secure-next-auth.session-token.1={_token1.value}",
            "accept": "*/*"
        }
